...
'''
#ID
time.sleep(3)search_input = driver.find_element(By.ID, "APjFqb")
...
parent_cell = driver.find_element(By.XPATH, "//table[@id='table1']//td[text()='jdoe@hotmail.com']")'''
...
print(f"to rightof Example -> Element to the right of twitter icon has href: {facebook_icon.get_attribute('href')}")

left_icon = driver.find_element(locate_with(By.TAG_NAME,"a").to_left_of(facebook_icon))
print(f"to leftof Example -> Element to the left of facebook icon has href: {left_icon.get_attribute('href')}")

near_twitter  = driver.find_elements(locate_with(By.TAG_NAME,"a").near(facebook_icon))
for elements in near_twitter:
    print(f"Near Example -> Element near Facebook icon href: {elements.get_attribute('href')}")
